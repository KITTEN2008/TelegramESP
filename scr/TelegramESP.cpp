...
#define CMD(STR)      buildCommand(F(STR))

TelegramESP::TelegramESP(const String& token, Client &client) {
  updateToken(token);
  this->client = &client;
}

void TelegramESP::updateToken(const String& token) {
  _token = token;
}

String TelegramESP::getToken() {
  return _token;
}

String TelegramESP::buildCommand(const String& cmd) {
  String command;
...
String TelegramESP::sendGetToTelegram(const String& command) {
  String body, headers;
...
bool TelegramESP::readHTTPAnswer(String &body, String &headers) {
  int ch_count = 0;
...
String TelegramESP::sendPostToTelegram(const String& command, JsonObject payload) {
...
String TelegramESP::sendMultipartFormDataToTelegram(
...
int TelegramESP::getUpdates(long offset) {

  #ifdef TELEGRAM_DEBUG  
    Serial.println(F("GET Update Messages"));
  #endif
  String command = CMD("getUpdates?offset=");
  command += offset;
  command += F("&limit=");
...
bool TelegramESP::sendMessage(const String& chat_id, const String& text,
...
bool TelegramESP::sendPostMessage(JsonObject payload, bool edit) { // added message_id
  ...
  if (payload.containsKey("text")) {
    while (millis() < sttime + 8000) { // loop for a while to send the message
        String response = sendPostToTelegram((edit ? CMD("editMessageText") : CMD("sendMessage")), payload); // if edit is true we send a editMessageText CMD
         #ifdef TELEGRAM_DEBUG  
        Serial.println(response);
      #endif
      sent = checkForOkResponse(response);
...
String TelegramESP::sendPostPhoto(JsonObject payload) {
  ...
  if (payload.containsKey("photo")) {
    while (millis() - sttime < 8000ul) { // loop for a while to send the message
      response = sendPostToTelegram(CMD("sendPhoto"), payload);
      #ifdef TELEGRAM_DEBUG  
        Serial.println(response);
      #endif
      sent = checkForOkResponse(response);
...
String TelegramESP::sendPhotoByBinary(
...
String TelegramESP::sendPhoto(const String& chat_id, const String& photo,
...
bool TelegramESP::checkForOkResponse(const String& response) {
  int last_id;
...
bool TelegramESP::sendChatAction(const String& chat_id, const String& text) {
...
bool TelegramESP::getFile(String& file_path, long& file_size, const String& file_id)
{
  String command = CMD("getFile?file_id=");
  command += file_id;
  String response = sendGetToTelegram(command); // receive reply from telegram.org
  DynamicJsonDocument doc(maxMessageLength);
  DeserializationError error = deserializeJson(doc, ZERO_COPY(response));
...
bool TelegramESP::answerCallbackQuery(const String &query_id, const String &text, bool show_alert, const String &url, int cache_time) {
  DynamicJsonDocument payload(maxMessageLength);
